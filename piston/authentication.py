...
forms = __import__(name = settings.PROJECT_NAME + '.piston', fromlist = ['forms',])
...
#       request.META['CONTENT_TYPE'] == "application/x-www-form-urlencoded":
    ...
    logger.info('request.META["Authorization"] is ' + request.META['Authorization'])
...
def oauth_access_token(request):
    ...
    """
    okey it get the oauth_server and oauth_request
    logger.info('oauth_server is ' + str(oauth_server))
    logger.info('oauth_request is ' + str(oauth_request))
    """
    ...
    try:
        #logger.info('in oauth_access_token, it get the token now')
        """
         the error must be raise in fetch_access_token,
         but not raise OAuthError
        """
        ...
        return HttpResponse(token.to_string())
            ...
            #logger.info('in oauth_access_token, raise OauthError, err is ' + err)
            return send_oauth_error(err)
...
class OAuthAuthentication(object):
    ...
    def is_valid_request(request):
        ...
        in Authorization header, it must contain this parameters:
        oauth_conusmer_key: the consumer key
        oauth_token: the acces token
        oauth_signature: the signature
        oauth_signature_method: the signature method, recommend HAMC-SHA1
        oauth_timestamp: the seconds from 1970.1.1
        oauth_nonce: the random string
        """
